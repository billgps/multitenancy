    ...
    <div class="w-full sm:px-6">
        ...
        <section class="flex flex-col break-words bg-white sm:border-1">
            <div class="col-span-6 h-12 flex items-center py-2 px-4 bg-gray-200">
                <a href="{{ route('room.edit', ['room' => $room->id]) }}" class="mx-2 text-yellow-600 hover:text-gray-400 modal-open image-toggle">
                    <i class="fas fa-edit"></i>
                </a>   
                <a href="{{ route('room.delete', ['room' => $room->id]) }}" class="mx-2 text-red-600 hover:text-gray-400 modal-open image-toggle">
            ...
            <form class="mr-auto my-auto space-y-6 sm:px-12 sm:pb-6"  method="POST"
                ...
                <div class="block sm:px-16">
                    <div class="flex flex-wrap mb-3">
                        <label class="block mb-2 text-sm text-gray-00" for="standard_name">Nama Ruangan</label>
                        <div class="relative flex items-center h-10 w-full input-component">
                            <input value="{{ $room->room_name }}" class="border-none outline-none w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="standard_name" name="standard_name" type="text" disabled>
                        </div>
                    </div>
                    <div class="flex flex-wrap mb-3">
                        <label class="block mb-2 text-sm text-gray-00" for="alias_name">Unit</label>
                        <div class="relative flex items-center h-10 w-full input-component">
                            <input value="{{ $room->unit }}" class="border-none outline-none w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="alias_name" name="alias_name" type="text" disabled>
                        </div>
                    </div>
                    <div class="flex flex-wrap mb-3">
                        <label class="block mb-2 text-sm text-gray-00" for="risk_level">Gedung</label>
                        <div class="relative flex items-center h-10 w-full input-component">
                            <input value="{{ $room->building }}" class="border-none outline-none w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="risk_level" name="risk_level" type="text" disabled>
                        </div>
                    </div>
                    <div class="flex flex-wrap row-start-3 mb-3">
                        <label class="block mb-2 text-sm text-gray-00" for="ipm_frequency">PIC Ruangan</label>
                        <div class="relative flex items-center h-10 w-full input-component">
                            <input value="{{ $room->room_pic }}" class="border-none outline-none w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="ipm_frequency" name="ipm_frequency" type="text" disabled>
                        </div>
                    </div>
                </div>
