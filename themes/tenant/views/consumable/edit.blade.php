    ...
    <div class="mx-auto w-4/5 sm:px-6">
        ...
        <section class="flex flex-col break-words bg-white sm:border-1">
            ...
            <form class="w-3/5 mx-auto pb-6 my-6" method="POST"
                action="{{ route('consumable.update', ['consumable' => $consumable->id]) }}">
                @csrf
                <div class="sm:grid sm:grid-cols-2 sm:gap-2 sm:px-6">
                    <div class="col-span-2">
                        ...
                        <div class="py-2 text-left flex">
                            ...
                            <script>
                                ...
                                $('#inventory_id').val({!! json_encode($consumable->inventory_id) !!})
                    ...
                    <div class="">
                        ...
                        <div class="py-2 text-left">
                            <input value="{{ $consumable->component }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="component" name="component" type="text" required>
                        </div>
                    </div>
                    <div class="">
                        <label class="block mb-2 text-sm text-gray-00" for="brand">Merk</label>
                        <div class="py-2 text-left">
                            <input value="{{ $consumable->brand }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="brand" name="brand" type="text" required>
                        </div>
                    </div>
                    <div class="">
                        <label class="block mb-2 text-sm text-gray-00" for="details">Keterangan</label>
                        <div class="py-2 text-left">
                            <textarea rows="4" cols="16" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="details" name="details" required>{{ $consumable->details }}</textarea>
                        </div>
                    </div>
                </div>
